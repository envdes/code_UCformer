...
from utils.tools import set_seed, split_and_rename, calculate_r2, calculate_rmse, cal_mess
import argparse
import pandas as pd
...
# =========== 3. 数据 ===========
# parquet_path = "/home/jiyang/Project_ucformer/rebuttel/ucformer/data/"   # 修改为你2070年数据的目录
batch_size = 128
train_loader, val_loader, tes_loader, scaled_train_lab_min, scaled_train_lab_max, train_lab_mean, train_lab_std = get_data(data_path, batch_size)
...
with torch.no_grad():
    for X, Y in tes_loader:
        ...
        Y = Y.to(device)
        output = model(X)
        # 逆缩放、逆标准化（先逆缩放再逆z-score）
        output_real = inverse_scale(output, scaled_train_lab_min, scaled_train_lab_max)
        output_real = inverse_standardize(output_real, train_lab_mean, train_lab_std)
        Y_real = inverse_scale(Y, scaled_train_lab_min, scaled_train_lab_max)
        Y_real = inverse_standardize(Y_real, train_lab_mean, train_lab_std)
        for i in range(3):
            y_pred[i].extend(output_real[:, :, i].cpu().numpy().flatten().tolist())
            y_true[i].extend(Y_real[:, :, i].cpu().numpy().flatten().tolist())

y_pred_df = pd.DataFrame(y_pred).T
y_true_df = pd.DataFrame(y_true).T
y_pred_df_splits = split_and_rename(y_pred_df)
y_true_df_splits = split_and_rename(y_true_df)
datm_data_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data', 'datm_data.csv')
datm_data = pd.read_csv(datm_data_path)
datm_data_splits = split_and_rename(datm_data)
all_T_rmse = []
all_q_rmse = []
all_t_rmse = []
all_T_mess = []
...
all_q_r2 = []
all_t_r2 = []
for i, city in enumerate(['London', 'NewYork', 'Shanghai', 'Singapore', 'SaoPaulo', 'Rome']):
